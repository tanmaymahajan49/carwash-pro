...
public class JwtAuthFilter implements GlobalFilter {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		if (request.getURI().getPath().contains("/login") || request.getURI().getPath().contains("-register") || request.getURI().getPath().contains("/booking/allCrops")) {
	...
	private boolean isAuthorized(String path, String role) {
		if (path.startsWith("/customer") && role.equals("CUSTOMER"))
			return true;
		if (path.startsWith("/washer") && role.equals("WASHER"))
			return true;
